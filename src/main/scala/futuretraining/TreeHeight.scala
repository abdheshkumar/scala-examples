...
object TreeHeight extends App {
  class Tree(val x: Int, val l: Tree, val r: Tree)

  object Tree {
    def apply(x: Int, l: Option[Tree], r: Option[Tree]): Tree =
      new Tree(x, l.orNull, r.orNull)
  }
  ...
  val tree = Tree(
    5,
    Some(Tree(3, Some(Tree(20, None, None)), Some(Tree(21, None, None)))),
    Some(Tree(10, Some(Tree(1, None, None)), None))
  )
